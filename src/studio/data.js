...
import { createReadmeModel } from "../core/model.js";
...
export async function studioSnapshot(root, configPath) {
  const loaded = await loadConfig(root, configPath);
  const facts = await inspectRepository({ root });
  const model = createReadmeModel(facts, loaded.config);
  ...
  return {
    ...
    config: loaded.found ? configPath : null,
